// document.addEventListener("DOMContentLoaded", function () {
      ...
      .then(function (data) {
        ...
        if (teams.length === 0) {
          ...
          const selectedTeam = teams.find(
            (team) => team.teamAbv === teamAbbreviation.toUpperCase()
          );
  ...
  fetch(
    "https://api.the-odds-api.com/v4/sports/?apiKey=d272dd2fc8c59ce6fb029378e95778ad"
  )
    .then(function (response) {
      return response.json();
    })
  ...
  function getSports() {
    ...
    fetch(
      `https://cors-anywhere.herokuapp.com/https://api.the-odds-api.com/v4/sports?apiKey=${apiKey}`
    )
      .then((response) => response.json())
      .then((data) => {
        const filteredSports = data.filter(
          (sport) => sport.group.toUpperCase() === userSport.toUpperCase()
        );
  ...
  function getOdds() {
    ...
    const market = prompt(
      "Enter the market (choices = h2h, spreads, totals, outrights):"
    );
  
    fetch(
      `https://cors-anywhere.herokuapp.com/https://api.the-odds-api.com/v4/sports/${sportsKey}/odds?apiKey=${apiKey}&regions=${region}&markets=${market}`
    )
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
  ...
  function getOddsHistory() {
    ...
    fetch(
      `https://cors-anywhere.herokuapp.com/https://api.the-odds-api.com/v4/sports/${sportsKey}/odds-history?apiKey=${apiKey}&regions=${region}&markets=${market}&date=${date}`
    )
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
